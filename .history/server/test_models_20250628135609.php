...
try {
    ...
    echo "✅ Movie created successfully! ID: " . $movie->getId() . "\n";
...
try {
    ...
    if ($foundMovie) {
        echo "✅ Movie found! Title: " . $foundMovie->getTitle() . "\n\n";
    } else {
...
try {
    ...
    echo "   Showtime ID: " . $seatLock->getShowtimeId() . "\n";
    echo "   Seat ID: " . $seatLock->getSeatId() . "\n";
    echo "   Expires: " . $seatLock->getExpiresAt() . "\n\n";
} catch (Exception $e) {
...
try {
    ...
    if ($foundLock) {
        ...
        echo "   Locked by user: " . $foundLock->getLockedByUser() . "\n\n";
    } else {
...
try {
    ...
    if ($lock) {
        $lock->setExpiresAt(date('Y-m-d H:i:s', strtotime('+30 minutes')));
...
try {
    ...
    foreach ($movies as $movie) {
        echo "   - " . $movie->getTitle() . " (ID: " . $movie->getId() . ")\n";
    }
    echo "\n";
} catch (Exception $e) {
